    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Login(LoginViewModel model, string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    var aaa = await _userManager.FindByNameAsync(model.Email);

                    var temp = this.HttpContext.Connection;

                    _dbContext.LoginLogs.Add(new LoginLog<string>() {
                        Id = Guid.NewGuid().ToString(), Description = "用户登录",
                        UserId = aaa.Id,
        ...
        [HttpGet]
        [HttpPost]
        public async Task<IActionResult> LoginLogs(int page = 1)
        {
            
            var user = await GetCurrentUserAsync();
            var list = new PageList<LoginLog<string>>(_dbContext.LoginLogs.OrderByDescending(o=>o.AddTime).Where(o => o.UserId == user.Id), page, 5);
